...
public class ControlPagosViewModel : PageViewModel
{
    ...
    public ControlPagosViewModel(ICamaraPortuariaReciboService recibos, IEmpresaRepository empresas, IDialogService dialog)
    {
        _recibos = recibos;
        _empresas = empresas;
        _dialog = dialog;
    ...
    private async Task MarcarMorosoAsync()
    {
        ...
        if (!await _dialog.ShowConfirmAsync("Estado moroso",
                $"¿Desea {accion} la empresa {Seleccionado.Empresa}?")) return;
        await _empresas.SetMorosoAsync(Seleccionado.EmpresaId, nuevoEstado);
        MostrarExito(nuevoEstado ? "Empresa marcada como morosa." : "Estado moroso removido.");
        await BuscarAsync();
    }
}
