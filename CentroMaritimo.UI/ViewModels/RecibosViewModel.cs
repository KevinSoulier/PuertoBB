...
public class RecibosViewModel : PageViewModel
{
    ...
    public RecibosViewModel(ICentroMaritimoReciboService service, IReciboRepository recibosRepo, IAgenciaRepository agenciasRepo, IDialogService dialog)
    {
        _service = service;
    ...
    private async Task EmitirIndividualAsync()
    {
        ...
        if (!await _dialog.ShowConfirmAsync("Emitir recibo",
                $"¿Emitir recibo a {AgenciaEmision.Nombre} por {ImporteEmision:C2}?")) return;

        IsBusy = true;
        try
        {
            var res = await _service.EmitirIndividualAsync(AgenciaEmision.Id, ImporteEmision, DetalleEmision, Anio, Mes);
